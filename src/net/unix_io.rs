use super::unix::{UnixStream, UnixStreamInner};
use std::io;
use std::pin::Pin;
...
fn poll_read(
    inner: &mut UnixStreamInner,
    cx: &mut Context<'_>,
    buffer: &mut [u8],
) -> Poll<io::Result<usize>> {
    match inner {
        #[cfg(feature = "tokio")]
        UnixStreamInner::Tokio(stream) => {
        ...
        #[cfg(feature = "smol")]
        UnixStreamInner::Smol(stream) => {
            futures::io::AsyncRead::poll_read(Pin::new(stream), cx, buffer)
        }
        #[cfg(feature = "compio")]
        UnixStreamInner::Compio(stream) => {
...
fn poll_flush(inner: &mut UnixStreamInner, cx: &mut Context<'_>) -> Poll<io::Result<()>> {
    match inner {
        #[cfg(feature = "tokio")]
        UnixStreamInner::Tokio(stream) => ::tokio::io::AsyncWrite::poll_flush(Pin::new(stream), cx),
        #[cfg(feature = "smol")]
        UnixStreamInner::Smol(stream) => futures::io::AsyncWrite::poll_flush(Pin::new(stream), cx),
        #[cfg(feature = "compio")]
        UnixStreamInner::Compio(stream) => futures::io::AsyncWrite::poll_flush(stream.as_mut(), cx),
        #[cfg(not(any(feature = "tokio", feature = "smol", feature = "compio")))]
        _ => unreachable!(),
    }
}

fn poll_close(inner: &mut UnixStreamInner, cx: &mut Context<'_>) -> Poll<io::Result<()>> {
    match inner {
        #[cfg(feature = "tokio")]
        UnixStreamInner::Tokio(stream) => {
        ...
        #[cfg(feature = "smol")]
        UnixStreamInner::Smol(stream) => futures::io::AsyncWrite::poll_close(Pin::new(stream), cx),
        #[cfg(feature = "compio")]
        UnixStreamInner::Compio(stream) => futures::io::AsyncWrite::poll_close(stream.as_mut(), cx),
        #[cfg(not(any(feature = "tokio", feature = "smol", feature = "compio")))]
        _ => unreachable!(),
    }
}
